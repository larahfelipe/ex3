...
/**
 * Prisma's code for a unique constraint violation. On sign-up it can only mean
 * the email is taken: the other unique columns written (`users.id`,
 * `portfolios.userId`) hold an id generated by the same statement.
 */
...
export class UserRepository {
  ...
  /**
   * User and portfolio are one nested write, which Prisma runs in a single
   * transaction: a failure leaves neither behind. The unique index on email is
   * the only authority on whether the address is free, so of concurrent
   * sign-ups for one email exactly one succeeds and the others resolve to null.
   * The session version is returned so the caller can issue the first token.
   */
