...
| O proxy `/api/v1/assets` soma `totalBalance` e calcula `dominance` sobre `balance` | O proxy só encaminha a listagem, e nenhuma rota do web calcula valor financeiro. A coluna `Dominance` da tabela de ativos vem do `allocation` por ativo de `GET /v1/portfolio/allocation`, participação no valor de mercado da carteira inteira, e o total do rodapé vem do `investedValue` de `GET /v1/portfolio/overview`, na moeda base | `web/src/app/api/v1/assets/route.ts` |
...
| `GET /v1/transactions/:assetSymbol/count` conta as compras e vendas de um ativo, e a tabela da tela de ativos pede uma contagem por linha | Cada item de `GET /v1/assets` traz `transactionCount` (`{ buy, sell }`), as transações `BUY` e `SELL` do ativo na carteira, contadas para a página inteira numa única consulta agrupada; tipo sem transação conta zero. O endpoint e o proxy `/api/v1/transactions/[symbol]/count` foram removidos, e a tabela lê a contagem da própria linha, sem requisição por ativo | `backend/src/services/asset/GetAllAssetsService.ts` |
| Nenhuma rota devolve o perfil do chamador; o web guarda em `localStorage` o usuário do sign-in, e a tabela de ativos rotula os valores com uma moeda escolhida num seletor, sem conversão | `GET /v1/user` (autenticado; `200` `{ user: { id, name, email, createdAt, updatedAt } }`) devolve o perfil do chamador, sem colunas de credencial nem `isAdmin`. O proxy `GET /api/v1/user` repassa com o cookie de sessão, e a área protegida lê o perfil pelo React Query, sem `localStorage`. O seletor de moeda saiu: a tabela de ativos formata os valores na `baseCurrency` da carteira | `docs/authentication.md` |
| Nenhuma série histórica da carteira | `GET /v1/portfolio/performance` (query `portfolioId`, `range?` — `1M`, `3M`, `6M`, `1Y` (padrão), `YTD` ou `MAX` — e `benchmark?`, símbolo do catálogo; `200` `{ baseCurrency, from, to, series, benchmark? }`) devolve o valor da carteira no fechamento de cada dia negociado da janela, na moeda base. Cada ponto traz `date`, `value`, `investedValue`, `netContribution` e `twr`, o retorno ponderado no tempo acumulado desde o primeiro ponto, que não conta como ganho o aporte nem a retirada do dia. Dia em que falta o fechamento ou o câmbio de alguma posição detida fica fora da série, sem mudar o `200`. A janela termina no início do dia corrente em UTC, exclusivo, e `MAX` começa no dia da primeira transação do razão. Com `benchmark`, o corpo traz `{ symbol, currency, series }`, cada ponto com `close` e o retorno sobre o primeiro fechamento da janela; símbolo fora do catálogo responde `404 Instrument not found in catalog`. Query inválida responde `400`, e carteira de outro usuário `404` | `backend/src/domain/PortfolioPerformance.ts` |
