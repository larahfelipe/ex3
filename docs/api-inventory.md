...
| `Asset` com `amount` e `balance`, movidos por `increment`/`decrement` a cada escrita de transação | A posição (tabela `positions`) tem `quantity`, `averageCost` e `balance`, e as respostas de ativo trazem os três no lugar de `amount`. Criar, editar e excluir transação reconstroem a posição a partir das transações dela; `balance` mantém o significado anterior, a soma de `±quantity × unitPrice` | `docs/domain-model.md` |
...
| Não existem tipos além de `BUY`/`SELL`, e `executedAt` não existe | O banco guarda os 11 tipos do modelo de domínio, e a API aceita `BUY` e `SELL` até a reconstrução implementar os demais (TD-017). O razão segue `executedAt`, `createdAt`, `id`, e transação retroativa entra no razão pela sua data | `docs/domain-model.md` |
| Quantidades e valores de transação e posição em `Float` | `DECIMAL(38,18)`; as respostas de ativo trazem `quantity`, `averageCost` e `balance` em string. Valor que a coluna arredondaria recebe 400; escrita cujo razão passa por posição fora da coluna recebe 400 `POSITION_OUT_OF_RANGE`, e transação em moeda diferente das outras da posição, 400 `CURRENCY_MISMATCH`, sem gravar | `docs/domain-model.md` |
| A tela de ativos calcula o preço médio a partir das transações listadas, somando compras e vendas | Exibe `averageCost` e `balance` da posição sem recalcular; o formulário de transação envia `quantity` e `unitPrice` em string, `executedAt` e a `baseCurrency` da carteira como `currency` | `web/src/app/(protected)/assets/_components/add-asset-transaction-dialog.tsx` |
