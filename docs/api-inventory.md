...
| PATCH | `/v1/user` | sim | `{ name?, oldPassword?, newPassword? }` | `200` `{ user, message }` | `400` payload inválido e `Invalid password`, este sem `details` · `401` sem sessão |
...
* `code` — uma das oito categorias de `ErrorCategories` (`backend/src/config/Constants.ts`): `VALIDATION` (400 e 413), `AUTHENTICATION` (401), `AUTHORIZATION` (403), `NOT_FOUND` (404), `CONFLICT` (409), `DOMAIN` (422), `INFRASTRUCTURE` (429 e 503), `INTERNAL` (500). Categoria e status são eixos distintos, e o código é estável entre versões. Ver `docs/errors.md`.
...
| `POST /v1/asset` e o `newSymbol` do `PATCH /v1/asset/:symbol` aceitam qualquer símbolo válido | O símbolo precisa estar no catálogo, ou entre os instrumentos privados do chamador; fora dos dois, `404 Instrument not found in catalog` | `backend/src/services/asset/CreateAssetService.ts`, `backend/src/services/asset/UpdateAssetService.ts` |
| Não há catálogo de instrumentos | `GET /v1/instruments` (query `page?`, `limit?` e `search?`; `200` `{ pagination, instruments }`, ordenado por símbolo) para qualquer usuário autenticado, e `search` filtra por prefixo do símbolo ou trecho do nome, sem diferenciar caixa, aceitando só letras, dígitos, espaço e `. & ' -`, para que `%` e `_` não virem curinga do `ILIKE`, e outro caractere responde `400`; `POST /v1/instrument` (`201` `{ instrument, message }`) só para admin, `403` aos demais; `market` aceita só `B3`, `NYSE`, `NASDAQ` e `CRYPTO`, e outro valor responde `400` | `backend/src/routes/InstrumentRoutes.ts` |
| Não há registro de instrumento fora do catálogo administrado | `POST /v1/asset` aceita `listing` opcional no corpo (`market`, `currency`) e, com ele, registra como instrumento privado do chamador o que o provedor de cotação lista sob o `symbol` naquele mercado e moeda — nome, classe e setor vêm do provedor, nunca do corpo — e cria o ativo sobre ele, na mesma escrita; sem `listing`, o símbolo precisa resolver a um instrumento já visível ao chamador (catálogo ou instrumento privado próprio). Símbolo já no catálogo responde `409 ALREADY_EXISTS`; símbolo já privado do próprio chamador, `409 PRIVATE_ALREADY_EXISTS`; moeda fora da moeda do mercado informado, `422 CURRENCY_MISMATCH` — os três sem consultar o provedor. Símbolo que o provedor não lista naquele mercado e moeda responde `404 NOT_LISTED`, e provedor indisponível, `503 UNAVAILABLE`, sem gravar. O `PATCH /v1/asset/:symbol` não recebe `listing`: o `newSymbol` só resolve a um instrumento que já existisse, no catálogo ou privado do chamador. Instrumento privado de outro usuário é sempre invisível, como se não existisse (`404`). Ver `docs/domain-model.md`, §Identidade e §Quem escreve | `backend/src/services/asset/CreateAssetService.ts`, `backend/src/validation/schema/asset/CreateAssetSchema.ts` |
| `PATCH /v1/instrument/:symbol` exigia admin para qualquer símbolo; não havia campo de escopo nem endpoint de opções | `GET /v1/instruments` traz `scope` em cada item, `CATALOG` ou `PRIVATE`; a listagem do chamador é o catálogo inteiro menos os símbolos que um instrumento privado seu sombreia, mais os seus próprios privados — nunca o privado de outro usuário. `PATCH` segue exigindo admin para um símbolo `CATALOG`, mas um símbolo `PRIVATE` só o próprio dono edita, admin incluído; outro chamador recebe `403`. `GET /v1/instruments/options`, que alimentava o cadastro manual, saiu com ele. O web busca pelo proxy `GET /api/v1/instruments/search` (linha abaixo); não há proxy para `GET /v1/instruments`, `POST /v1/instrument` nem para o `PATCH`, que ficam de fora da interface (TD-068) | `backend/src/domain/InstrumentCatalog.ts`, `backend/src/services/instrument/UpdateInstrumentService.ts` |
| Adicionar ativo fora do catálogo exigia digitar nome, classe, mercado, moeda e setor | `GET /v1/instruments/search` (query `query`, com o padrão e o limite de `search`; `200` `{ instruments, listings, marketSearch }`) devolve até 20 instrumentos visíveis ao chamador sob o termo e, quando o termo pode ser um símbolo novo (até 6 letras ou dígitos) e não nomeia instrumento que ele já vê, o que o provedor de cotação lista sob ele, como `{ symbol, name, type, market, currency }`. `marketSearch` diz se o provedor foi consultado (`SEARCHED`), dispensado (`SKIPPED`) ou estava indisponível (`UNAVAILABLE`); indisponível não falha a busca. Limitada a 30 por minuto por usuário (`429`), além do budget geral, porque cada termo pode gastar a cota do provedor. Termo ausente, vazio, repetido ou fora do padrão responde `400` | `backend/src/services/instrument/SearchInstrumentsService.ts`, `backend/src/routes/InstrumentRoutes.ts` |
| Um portfolio por usuário (`Portfolio.userId @unique`); `GET /v1/portfolio` devolve o do chamador e `GET /v1/portfolios` lista todos, sem paginação, só para admin | Cada usuário tem N carteiras, com `name` e `baseCurrency`. `GET /v1/portfolios` (query `page?`, `limit?`; `200` `{ pagination, portfolios }`, em ordem de criação) lista só as do chamador; `GET /v1/portfolio` (query `portfolioId`; `200` `Omit<Portfolio,'assets'>`) devolve uma delas; `POST /v1/portfolio` (`{ name, baseCurrency }`; `201` `{ portfolio, message }`) cria outra; `PATCH /v1/portfolio` (`{ portfolioId, name?, baseCurrency? }`, ao menos um atributo; `200` `{ portfolio, message }`) renomeia ou troca a moeda base, que só muda enquanto a carteira não tem transação (`422`); nos dois, o nome é gravado em NFC com os espaços colapsados, e nome que o chamador já usa, em qualquer caixa ou espaçamento, responde `409 You already have a portfolio with this name`, com `details` em `name` (`docs/domain-model.md`, §Carteira); `DELETE /v1/portfolio` (query `portfolioId`; `200` `{ message }`) exclui a carteira com posições e transações, menos a última da conta (`422`). Carteira de outro usuário responde `404 Portfolio not found for this user`, como inexistente | `backend/src/routes/PortfolioRoutes.ts` |
...
| `POST /v1/user/create` cria o `Portfolio` do usuário no mesmo fluxo (1:1) | O corpo exige `baseCurrency` (ISO 4217) e aceita `portfolioName` opcional, com os limites do nome de carteira; a primeira carteira leva esse nome, ou `Main` sem ele, nessa moeda, gravada na mesma escrita do usuário | `docs/authentication.md` |
| Sem proxy para `/v1/portfolio` e `/v1/portfolios` | `GET /api/v1/portfolios` repassa a query ao backend, `POST /api/v1/portfolios/create` encaminha para `POST /v1/portfolio`, `GET`, `PATCH` e `DELETE` `/api/v1/portfolio` encaminham para `/v1/portfolio` com o `portfolioId` na query ou no corpo, e os proxies de ativo e de transação repassam a query string com o `portfolioId`. O web opera a carteira ativa, escolhida na tela de carteiras, e posta transação em `/api/v1/transactions/create`; antes usava `/api/v1/transactions`, que não tem handler | `web/src/app/api/v1/portfolio/route.ts` |
| `Asset` com `amount` e `balance`, movidos por `increment`/`decrement` a cada escrita de transação | A posição (tabela `positions`) tem `quantity`, `averageCost` e `investedValue`, e as respostas de ativo trazem os três no lugar de `amount` e `balance`. Criar, editar e excluir transação reconstroem a posição a partir das transações dela; `investedValue` é o custo das unidades detidas, `quantity × averageCost`. O valor de mercado não entra nessas respostas: vem de `GET /v1/portfolio/positions` | `docs/domain-model.md` |
...
| `GET /v1/transactions/:assetSymbol/count` conta as compras e vendas de um ativo, e a tabela da tela de ativos pede uma contagem por linha | A contagem virou um campo do item da listagem de ativos, e depois a listagem inteira saiu: `GET /v1/assets`, o endpoint de contagem e os proxies `/api/v1/assets` e `/api/v1/transactions/[symbol]/count` foram removidos. A tela de ativos lê `GET /v1/portfolio/positions` (TD-034) | `backend/src/routes/AssetRoutes.ts` |
...
| Nenhum indicador de uma posição além do valor e do resultado não realizado | `GET /v1/portfolio/positions/:symbol/indicators` (query `portfolioId`; `200` `{ prices?, returns? }`) descreve uma posição da carteira, com ou sem unidades. `prices`, dos fechamentos diários do último ano na moeda da cotação, traz `currency`, `close`, `closedOn`, `yearLow`, `yearHigh`, `changes`, a variação de `1M`, `3M`, `6M`, `YTD` e `1Y`, cada uma sem `change` quando o histórico não a alcança e, com `change`, o dia da abertura em `openedOn`, e `closes`, um fechamento por dia (`close`, `closedOn`) em ordem crescente, da abertura de `1Y`, ou do primeiro fechamento quando o histórico é mais curto, até o mais recente; fica fora do corpo sem fechamento no último ano. `returns`, do razão executado até o instante da requisição na moeda dele, traz `currency`, `since`, `realizedProfitLoss`, `income`, `trailingIncome` e, com valor investido, `yieldOnCost`; fica fora do corpo sem transação. Fechamentos que faltam são pedidos ao provedor e gravados, como na performance, e o provedor indisponível deixa `prices` com o que está gravado. O símbolo é normalizado como na escrita. Símbolo sem posição na carteira responde `404 Asset not found in portfolio`, sem ler histórico; query ou símbolo inválido, `400`; carteira de outro usuário, `404`. O web lê pelo proxy `GET /api/v1/portfolio/positions/[symbol]/indicators`, com o hook `usePositionIndicators` | `docs/financial-rules.md` |
| Nenhum indicador fundamentalista | `GET /v1/portfolio/positions/:symbol/fundamentals` (query `portfolioId`; `200` `{ outcome, source?, figures? }`) descreve o instrumento de uma posição pelos indicadores que a classe dele comporta. `outcome` é `reported`, com `source`, a fonte que os calculou, e `figures`, um `{ metric, value? }` por indicador da classe, na ordem de exibição, sem `value` quando a fonte não o informa, e `freeCashFlow` com `currency`; `not-applicable`, para classe sem indicador, sem consultar o provedor; `not-found`; ou `unavailable`, sem mudar o `200`. O símbolo é normalizado como na escrita. Símbolo sem posição na carteira responde `404 Asset not found in portfolio`, sem consultar o provedor; query ou símbolo inválido, `400`; carteira de outro usuário, `404`. O web lê pelo proxy `GET /api/v1/portfolio/positions/[symbol]/fundamentals`, com o hook `usePositionFundamentals` | `docs/financial-rules.md` |
...
| `PATCH`/`DELETE` de user sem rota proxy; a tela de conta exibia o perfil sem poder alterá-lo | `PATCH /api/v1/user` (cookie) encaminha para `PATCH /v1/user`. Quando o corpo traz `newPassword` e a API responde `2xx`, o proxy apaga o cookie `ex3:token` na mesma resposta, porque a troca revogou a sessão; a tela de conta edita o nome e troca a senha por ele. `DELETE /v1/user` segue sem proxy | `web/src/app/api/v1/user/route.ts` |
...
| Nenhuma rota informa se a instância está de pé ou apta a servir | `GET /health` (sem autenticação, fora de `/v1`; `200` `{ status: 'alive' }`) responde sem tocar dependência alguma, e `GET /ready` (idem; `200` `{ status: 'ready', database: 'up' }`) consulta o banco a cada chamada e responde `503 INFRASTRUCTURE` quando ele não atende, sem dizer ao chamador o que falhou. As duas ficam atrás do rate limit da API. O `healthcheck` do `backend` no `compose.yaml` chama `/ready`; em produção, nada as sonda ainda (TD-060) | `docs/observability.md` |
