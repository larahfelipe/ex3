...
| Nenhuma rota informa se a instância está de pé ou apta a servir | `GET /health` (sem autenticação, fora de `/v1`; `200` `{ status: 'alive' }`) responde sem tocar dependência alguma, e `GET /ready` (idem; `200` `{ status: 'ready', database: 'up' }`) consulta o banco a cada chamada e responde `503 INFRASTRUCTURE` quando ele não atende, sem dizer ao chamador o que falhou. As duas ficam atrás do rate limit da API. Nada no repositório as sonda ainda (TD-060) | `docs/observability.md` |
