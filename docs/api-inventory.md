...
| Não há registro de instrumento fora do catálogo administrado | `POST /v1/asset` aceita `instrument` opcional no corpo (`name`, `type`, `market`, `currency`, `sector?`, `country?`) e, com ele, registra um instrumento privado do chamador sob o `symbol` informado e cria o ativo sobre ele, na mesma escrita; sem `instrument`, o símbolo precisa resolver a um instrumento já visível ao chamador (catálogo ou instrumento privado próprio). Símbolo já no catálogo responde `409 ALREADY_EXISTS`; símbolo já privado do próprio chamador, `409 PRIVATE_ALREADY_EXISTS`; moeda fora da moeda do mercado informado, `422 CURRENCY_MISMATCH`, sem gravar. O `PATCH /v1/asset/:symbol` não recebe `instrument`: o `newSymbol` só resolve a um instrumento que já existisse, no catálogo ou privado do chamador. Instrumento privado de outro usuário é sempre invisível, como se não existisse (`404`). Ver `docs/domain-model.md`, §Identidade e §Quem escreve | `backend/src/services/asset/CreateAssetService.ts`, `backend/src/validation/schema/instrument/InstrumentAttributesSchema.ts` |
| `PATCH /v1/instrument/:symbol` exigia admin para qualquer símbolo; não havia campo de escopo nem endpoint de opções | `GET /v1/instruments` traz `scope` em cada item, `CATALOG` ou `PRIVATE`; a listagem do chamador é o catálogo inteiro menos os símbolos que um instrumento privado seu sombreia, mais os seus próprios privados — nunca o privado de outro usuário. `PATCH` segue exigindo admin para um símbolo `CATALOG`, mas um símbolo `PRIVATE` só o próprio dono edita, admin incluído; outro chamador recebe `403`. `GET /v1/instruments/options` (`200` `{ types, markets }`, sem paginação; `types` é a lista de `InstrumentType`, `markets` é `{ market, currency }` por mercado conhecido, `currency` nulo quando o mercado aceita qualquer uma, como `CRYPTO`) alimenta os campos de registro sem fixar a lista no cliente. O web lê as duas pelos proxies `GET /api/v1/instruments` e `GET /api/v1/instruments/options`; não há proxy para `POST /v1/instrument` nem para o `PATCH`, que ficam de fora da interface (TD-068) | `backend/src/domain/InstrumentCatalog.ts`, `backend/src/services/instrument/UpdateInstrumentService.ts` |
