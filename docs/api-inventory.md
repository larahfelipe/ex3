...
| `Asset` com `amount` e `balance`, movidos por `increment`/`decrement` a cada escrita de transação | A posição (tabela `positions`) tem `quantity`, `averageCost` e `investedValue`, e as respostas de ativo trazem os três no lugar de `amount` e `balance`. Criar, editar e excluir transação reconstroem a posição a partir das transações dela; `investedValue` é o custo das unidades detidas, `quantity × averageCost`, e `GET /v1/assets` com `sort` ordena por ele, com `sort.field` `investedValue`. `marketPrice`, `marketValue`, `profitLoss` e `profitLossPercent` ainda não são expostos, porque não há cotação | `docs/domain-model.md` |
