...
**Quem escreve.** Só admin cadastra e corrige instrumentos do catálogo; a escrita de outro usuário no catálogo recebe 403. Qualquer usuário autenticado registra instrumento privado ao abrir o ativo (`POST /v1/assets/create` com `listing`), a partir do que o provedor de cotação lista sob o símbolo no mercado e na moeda escolhidos: nome, classe e setor nunca vêm do cliente. Só o dono o corrige. O cadastro recusa com `409` o símbolo que o catálogo já tem (`ALREADY_EXISTS`), porque o ativo deve vir do catálogo, e o que o usuário já cadastrou (`PRIVATE_ALREADY_EXISTS`). Instrumento privado de outro usuário é invisível: listá-lo, abri-lo como ativo ou corrigi-lo responde como símbolo inexistente, 404. O web adiciona ativo por uma busca só: os instrumentos que o usuário já vê, com o selo *Private* nos dele, e as listagens do provedor sob o termo como ticker, com o selo *New*.
...
* Cada requisição expira em 5 segundos. Status de erro, timeout, falha de rede ou resposta fora do formato suspendem as chamadas ao provedor por 30 segundos; na falha e durante a pausa, cada símbolo responde a última cotação recebida, com o `timestamp` em que foi observada, ou `unavailable` sem cotação anterior. O histórico não usa o cache das cotações.
* Uma consulta de histórico igual a uma em curso — mesmo símbolo, intervalo e janela — aproveita a requisição em curso. A janela que o provedor respondeu sem preço, ou com 404, é respondida igual por uma hora sem nova requisição, até 10.000 janelas, descartando a mais antiga. Janela com preço e falha não ficam guardadas.
* A cotação corrente não é gravada; o que a tabela guarda é o fechamento diário (ver [Cotações gravadas](#cotações-gravadas)).
* A busca de listagens pede numa só requisição de cotação o ticker em cada mercado — `{symbol}.SA`, `{symbol}` e `{symbol}-{currency}` para `BRL`, `USD` e `EUR` — e aceita só o item cuja bolsa e moeda confirmam o mercado pedido: `SAO` é `B3`; `NYQ`, `ASE` e `PCX` são `NYSE`; `NMS`, `NGM` e `NCM` são `NASDAQ`; `CCC` é `CRYPTO`. A classe vem de `quoteType` (`EQUITY` é `STOCK`, `ETF` é `ETF`, `MUTUALFUND` é `FUND`, `CRYPTOCURRENCY` é `CRYPTO`), e ação da B3 com `FII` ou `imobiliári` no nome é `REIT`. O nome é `longName`, senão `shortName`, com os espaços colapsados e cortado em 120 caracteres.
* A busca fica em cache por uma hora, até 1.000 tickers, descartando o mais antigo; durante uma falha, responde a última busca guardada. Falha da busca pausa o provedor como a da cotação.
* O setor vem de `/v11/finance/quoteSummary/{symbol}?modules=assetProfile`, pedido só no registro de `STOCK` e `REIT`. Falha ou resposta fora do formato deixam o setor vazio, registram `quote_provider_request_failed` e não pausam as cotações.
* Os fundamentos vêm do mesmo caminho, só com os módulos dos indicadores pedidos: `summaryDetail` (`trailingPE`; `trailingAnnualDividendYield`, ou `yield` num fundo) e `financialData` (`returnOnEquity`, `profitMargins`, `debtToEquity`, `revenueGrowth`, `earningsGrowth`, `freeCashflow` na moeda `financialCurrency`). Cada número é lido como `{ raw }` ou número puro; `{}` ou valor ilegível deixam só aquele indicador sem valor, e fluxo de caixa sem moeda ISO 4217 fica de fora. `debtToEquity` vem em percentual e é devolvido como múltiplo. A resposta fica em cache por uma hora, por símbolo e módulos, inclusive o 404; falha ou resposta fora do formato respondem `unavailable`, registram `quote_provider_request_failed`, não pausam as cotações e não ficam no cache. Os nomes dos campos seguem o formato conhecido do `quoteSummary`, sem conferência contra o provedor real (TD-079).

Os valores de timeout, cache, pausa e lote são assumidos, não medidos, e a cota e o preço dos planos do provedor não foram verificados (TD-020). O mapa de bolsas e a regra de FII também são assumidos (TD-074).
