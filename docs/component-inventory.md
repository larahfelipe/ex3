...
| --- | --- |
| `components/sidebar.tsx` | **Refatorar.** Navegação hard-coded para `Overview`/`Assets`/`Portfolios`/`Account`, com o item ativo lido da rota por `usePathname`, também nas sub-rotas, como `Assets` no detalhe do ativo, e marcado com `aria-current="page"`; não suporta a árvore da TASK 13.1 nem tem equivalente mobile |
...
| `add-asset-dialog.tsx` | 359 | **Reescrito.** Monta o próprio formulário a cada abertura e escolhe o instrumento do catálogo, sem símbolo digitado: busca por símbolo ou nome em `GET /v1/instruments`, 300 ms depois da última tecla, com o termo validado pelo mesmo padrão da API, e até 20 resultados em botões de rádio nativos. O estado do catálogo é anunciado (`role="status"`): carregando, buscando, catálogo vazio, nenhum resultado, com atalho para limpar a busca, e mais resultados que os exibidos. Falha de carga mostra erro com nova tentativa, e falha do cadastro fica no diálogo (`role="alert"`), com o 409 traduzido para o ativo já existir na carteira, sem toast. Depois do cadastro, oferece registrar uma transação do ativo |
...
| Componente | Responsabilidade |
| --- | --- |
| `portfolios/page.tsx` | Tela de carteiras, sobre `QuerySection`, com `GET /v1/portfolios` 10 por página: nome, moeda base e a marca da carteira ativa em cada linha, com "Use", "Edit" e "Delete", cujos nomes acessíveis incluem o da carteira. "Use" guarda a escolha da carteira ativa, e o bloqueio do armazenamento pelo navegador vira toast de erro. Excluir fica desabilitado, com a explicação abaixo da lista, quando a conta tem uma só. Carregando, erro com nova tentativa, vazio com atalho para criar e página além da última com atalho para ela. Depois de excluir, o foco vai para "New portfolio", porque a linha que abriu o diálogo saiu |
...
| `delete-portfolio-dialog.tsx` | `DeletePortfolioDialog`, no padrão de `DeleteTransactionDialog`: avisa que ativos e transações saem junto e, sendo a ativa, que a tela passa à mais antiga; o erro, como o 422 da última carteira, fica no diálogo |
