...
| N6 | Sob `prefers-reduced-motion`, animação e transição não essenciais são neutralizadas em `globals.css`; o indicador de ocupado segue girando, mais devagar. O framer-motion segue a mesma preferência: `MotionScope` (`MotionConfig reducedMotion="user"`) desliga o deslize dos painéis e do indicador do `SegmentedControl`, que só trocam de lugar, e mantém o fade; a altura do painel, que não é transform, troca sem transição por `useReducedMotion`. Sob `prefers-reduced-transparency`, o overlay dos diálogos perde o desfoque e fica mais opaco | 2.3.3 · bloco `@media (prefers-reduced-motion: reduce)`; transform de toque sob `motion-safe:`; `MotionScope` e `SlideTransition` |
...
| A política de senha só aparecia depois do erro (3.3.2) | dica com os requisitos ligada por `aria-describedby` à senha do sign-up e à nova senha da conta; hoje é a lista de `PasswordRequirements`. Na tela de conta ela marca cada requisito cumprido e diz o estado em texto; no sign-up só lista os requisitos, porque lá a senha é julgada no envio |
| Sign-in e sign-up validavam em `onChange`, com erro a cada tecla antes de o campo ser deixado | `onTouched`, como nos diálogos. Revertido em 2026-09-23 a pedido do usuário: todo formulário valida em `onChange`, para que o campo só mostre erro depois de receber um valor, e não ao ser deixado vazio. O erro acompanha cada tecla, mas não é região viva — chega ao leitor de tela pelo `aria-describedby` do campo —, então não é anunciado a cada tecla. Em 2026-09-24, também a pedido do usuário, os campos de senha do sign-in e do sign-up passaram a validar só no envio (no sign-up, no avanço da etapa), sem revalidar enquanto se digita, para que o layout não se mova durante a digitação. O envio inválido foca o primeiro campo com erro, que fica com `aria-invalid` e a mensagem no `aria-describedby` |
| "Login", "Register", "Login instead", "Logged in as", "Logged out successfully" e dois toasts no sign-up, ao lado de "Sign out" no menu | "Sign in", "Create account", "Sign in instead", "Signed in as", "Signed out" e um toast só; conta sem nome é saudada pelo e-mail; os títulos das páginas, "Sign In" e "Sign Up", viraram "Sign in" e "Create account" |
