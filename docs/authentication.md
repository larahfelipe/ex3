...
| **Throttling** | Sign-in e sign-up usam o limite de autenticação (10 requisições a cada 15 min). `PATCH` e `DELETE /v1/user` também, porque verificam a senha: uma sessão roubada não pode virar oráculo de adivinhação de senha. O balde é por conta — o e-mail submetido, ou a sessão que apresenta o token, nunca o endereço do chamador, que em produção é sempre o do web | `routes/UserRoutes.ts`, `middleware/RateLimitMiddleware.ts` |
