...
O bloco `performance` de `src/routes/Portfolios.integration.ts` cobre `GET /v1/portfolio/performance` contra o banco, trocando `getHistoricalPrices` e `getHistoricalExchangeRate` do singleton por `t.mock.method`, como as demais rotas trocam a cotação: a série da carteira pedida na moeda base, com a janela que chegou ao provedor; a posição em moeda estrangeira convertida pela taxa de cada dia, com o par pedido uma vez; o benchmark ao lado da carteira; benchmark fora do catálogo com 404; carteira sem transação como janela vazia; carteira de outro usuário igual à inexistente; e `portfolioId`, `range` ou `benchmark` ausente ou malformado com 400. Os fechamentos são semeados relativos ao dia corrente, e não em datas fixas, que a suíte deixaria para trás.
